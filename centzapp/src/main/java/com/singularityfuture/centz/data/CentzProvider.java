 ...
 */
package com.singularityfuture.centz.data;
...
/**
 * This class serves as the ContentProvider for all of Centz's data. This class allows us to
 ...
 * so in this course.
 */
public class CentzProvider extends ContentProvider {
     ...
     */
    public static final int CODE_CENTZ = 100;
    public static final int CODE_CENTZ_WITH_DATE = 101;

    /*
     * The URI Matcher used by this content provider. The leading "s" in this variable name
     * signifies that this UriMatcher is a static member variable of CentzProvider and is a
     * common convention in Android programming.
     */
    private static final UriMatcher sUriMatcher = buildUriMatcher();
    private CentzDbHelper mOpenHelper;

    /**
     * Creates the UriMatcher that will match each URI to the CODE_CENTZ and
     * CODE_CENTZ_WITH_DATE constants defined above.
     * <p>
     ...
     * reason not to.
     *
     * @return A UriMatcher that correctly matches the constants for CODE_CENTZ and CODE_CENTZ_WITH_DATE
     */
    public static UriMatcher buildUriMatcher() {
        ...
        final String authority = CentzContract.CONTENT_AUTHORITY;

        /*
         * For each type of URI you want to add, create a corresponding code. Preferably, these are
         * constant fields in your class so that you can use them throughout the class and you no
         * they aren't going to change. In Centz, we use CODE_CENTZ or CODE_CENTZ_WITH_DATE.
         */

        /* This URI is content://com.singularityfuture.centz/centz/ */
        matcher.addURI(authority, CentzContract.PATH_CENTZ, CODE_CENTZ);

        /*
         * This URI would look something like content://com.singularityfuture.centz/centz/1472214172
         * The "/#" signifies to the UriMatcher that if PATH_CENTZ is followed by ANY number,
         * that it should return the CODE_CENTZ_WITH_DATE code
         */
        matcher.addURI(authority, CentzContract.PATH_CENTZ + "/#", CODE_CENTZ_WITH_DATE);
    ...
    public boolean onCreate() {
         ...
         * lengthy operations will cause lag in your app. Since CentzDbHelper's constructor is
         * very lightweight, we are safe to perform that initialization here.
         */
        mOpenHelper = new CentzDbHelper(getContext());
        return true;
    }

    /**
     * Handles requests to insert a set of new rows. In Centz, we are only going to be
     * inserting multiple rows of data at a time from a centz forecast. There is no use case
            ...
            case CODE_CENTZ:
                ...
                try {
                    for (ContentValues value : values) {
                        long centzDate =
                                value.getAsLong(CentzContract.CentzEntry.COLUMN_DATE);
                        if (!CentzDateUtils.isDateNormalized(centzDate)) {
                            throw new IllegalArgumentException("Date must be normalized to insert");
                        }

                        long _id = db.insert(CentzContract.CentzEntry.TABLE_NAME, null, value);
                        if (_id != -1) {
                            rowsInserted++;
                        }
                    }
    ...
    /**
     * Handles query requests from clients. We will use this method in Centz to query for all
     * of our centz data as well as to query for the centz on a particular day.
     *
    ...
    public Cursor query(@NonNull Uri uri, String[] projection, String selection,
        ...
        switch (sUriMatcher.match(uri)) {
             ...
             *
             *      content://com.singularityfuture.centz/centz/1472214172
             *
             * sUriMatcher's match method will return the code that indicates to us that we need
             * to return the centz for a particular date. The date in this code is encoded in
             ...
             *
             * In this case, we want to return a cursor that contains one row of centz data for
             * a particular date.
             */
            case CODE_CENTZ_WITH_DATE: {
                ...
                cursor = mOpenHelper.getReadableDatabase().query(
                        ...
                        projection,
                        /*
                         * The URI that matches CODE_CENTZ_WITH_DATE contains a date at the end
                         * of it. We extract that date and use it with these next two lines to
                         * specify the row of centz we want returned in the cursor. We use a
             ...
             *
             *      content://com.singularityfuture.centz/centz/
             *
             * sUriMatcher's match method will return the code that indicates to us that we need
             * to return all of the centz in our centz table.
             *
             * In this case, we want to return a cursor that contains every row of centz data
             * in our centz table.
             */
            case CODE_CENTZ: {
                cursor = mOpenHelper.getReadableDatabase().query(
    ...
    /**
     * In Centz, we aren't going to do anything with this method. However, we are required to
     * override it as CentzProvider extends ContentProvider and getType is an abstract method in
     ...
     *
     * @param uri the URI to query.
     * @return nothing in Centz, but normally a MIME type string, or null if there is no type.
     */
    @Override
    public String getType(@NonNull Uri uri) {
        throw new RuntimeException("We are not implementing getType in Centz.");
    }

    /**
     * In Centz, we aren't going to do anything with this method. However, we are required to
     * override it as CentzProvider extends ContentProvider and insert is an abstract method in
     * ContentProvider. Rather than the single insert method, we are only going to implement
     * {@link CentzProvider#bulkInsert}.
     *
     * @param uri    The URI of the insertion request. This must not be null.
     * @param values A set of column_name/value pairs to add to the database.
     *               This must not be null
     * @return nothing in Centz, but normally the URI for the newly inserted item.
     */
    @Override
    public Uri insert(@NonNull Uri uri, ContentValues values) {
        throw new RuntimeException(
                "We are not implementing insert in Centz. Use bulkInsert instead");
    }

    @Override
    public int update(@NonNull Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        throw new RuntimeException("We are not implementing update in Centz");
    }
